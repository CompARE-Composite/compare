...
<body>
    <h1>Number of Views: <span id="viewCount">Loading...</span></h1>

    <script>
        // Replace 'G-EGNE926NLF' with your Google Analytics view ID
        gtag('config', 'G-EGNE926NLF');

        // Function to fetch view count
        function fetchViewCount() {
            gtag('event', 'page_view', {
                'send_to': 'G-EGNE926NLF',
                'event_callback': function() {
                    var viewCount = gtag.getAll()[0].get('page_view_count');
                    document.getElementById('viewCount').textContent = viewCount;
                }
            });
        }

        // Fetch view count when the page loads
        fetchViewCount();
    </script>
</body>
```

# Number of Views: <span id="viewCount">Loading...</span>

<script>
  // Function to fetch view count
  function fetchViewCount() {
    // Make an API call to retrieve the view count
    // Replace 'G-EGNE926NLF' with the actual endpoint to fetch the view count
    fetch('G-EGNE926NLF')
