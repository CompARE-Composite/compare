...
We can use the information of thsi study to plan a new similar trial.
...
**components, it is not a problem beacuse using the CE as the primary endpoint** 
...
The required sample size for a trial with CE as primary endpoint depends on the 
correlation &#961;. In general, sample size increases as the correlation also 
increases. The observed decrease in sample size for high correlations is due to 
the fact that if we keep the proportion of observed events fixed, the events in 
the second component would need to occur almost entirely before the end of the 
follow-up period to avoid not being observed due to the first competitive event.
